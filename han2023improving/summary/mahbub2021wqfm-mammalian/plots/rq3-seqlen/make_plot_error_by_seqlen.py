...
def make_figure(df, output):
    ...
    ax00 = plt.subplot(gs[0,0])  ## changing seqlen

    # Plot error for varying sequence length
    ax = ax00
    varys = ["250b", "500b", "1000b", "1500b", "true"]
    vnams = ["250", "500", "1000", "1500", "true"]
    n = len(varys)
    ...
    # Set labels
    #ax.set_title("Impact of GTEE on Mammalian 1X (37 taxa, 200 gene trees)", 
    #             loc="center", y=1.1, fontsize=12)
    ax.set_title(upperletters[0] + str(" Mammalian 1X"),
                 loc="left", fontsize=11, color="white")
    ax.set_ylabel(r"Percent RF Error", fontsize=11)  

    ax.tick_params(axis='x', labelsize=10)
    ax.tick_params(axis='y', labelsize=9)
    ax.set_xlabel("Sequence Length (Decreasing GTEE)", fontsize=12)
...
make_figure(df, "supplot-rq3-seqlen.pdf")
