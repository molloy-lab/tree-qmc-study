...
def make_figure(df, output):
    ...
    ax20 = plt.subplot(gs[2,0])  ## avian 0.5X - wqfit
    ax21 = plt.subplot(gs[2,1])  ## avian 1X - wqfit
    ax22 = plt.subplot(gs[2,2])  ## avian 2X - wqfit
    ...
    for i, scal in enumerate(scals):
        ...
        #ax.set_title(letters[i+2] + str(" Species tree height %s" % scal),
        #            loc="left", x=0.0, y=1.0, fontsize=10.5)
        ax.set_title(upperletters[i+6] + str(" Species tree scale %s" % scal),
                 loc="left", fontsize=11)
        if i == 0:
            ax.set_ylabel(r"wQfit", fontsize=11)  
        ...
        if scal == "0.5X":
            yticks = [0.86, 0.88, 0.90, 0.92, 0.94, 0.96, 0.98, 1.00]
        elif scal == "1X":
...
title = "supplot-rq2b-ils-avian-with-qscore-with-wqfit.pdf"
