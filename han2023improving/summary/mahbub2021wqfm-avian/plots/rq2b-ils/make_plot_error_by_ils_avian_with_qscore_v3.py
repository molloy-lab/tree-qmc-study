...
def make_figure(df, output):
    ...
    ax00 = plt.subplot(gs[0,0])  ## avian esti gtre - species tree error
    ax01 = plt.subplot(gs[0,1])  ## avian true gtre - species tree error

    ax10 = plt.subplot(gs[1,0])  ## avian esti gtre - quartet score
    ax11 = plt.subplot(gs[1,1])  ## avian true gtre - quartet score

    rows = [ax00, ax01]
    cols = [ax10, ax11]

    slens = ["500", "true"]
    gtres = ["1000 estimated gene trees (500 bp)", "1000 true gene trees"]
    varys = ["0.5X", "1X", "2X"] 
    vnams = ["0.5X", "1X", "2X"]
    ...
    for i, slen in enumerate(slens):
        ...
        ax.set_title(upperletters[i] + str(" %s" % gtres[i]),
    ...
    for i, slen in enumerate(slens):
        ...
        #            loc="left", x=0.0, y=1.0, fontsize=10.5)
        ax.set_title(upperletters[i+2] + str("  %s" % gtres[i]),
                 loc="left", fontsize=11)
        if i == 0:
        ...
        ax.tick_params(axis='x', labelsize=10)
        ax.tick_params(axis='y', labelsize=9)
        ax.set_xlabel("Species Tree Scale", fontsize=12)
        ax.set_xlim(xminor[0]-1, xminor[-1]+1)
        ax.set_xticks(xmajor)
        ...
        if slen == "500":
            ...
            yticks = [-0.6, -0.3, 0.0, 0.3, 0.6, 0.9]
        ydraw = yticks
        ymax = yticks[-1]
        ymin = -0.05 * ymax
        ax.set_ylim(ymin, ymax)
