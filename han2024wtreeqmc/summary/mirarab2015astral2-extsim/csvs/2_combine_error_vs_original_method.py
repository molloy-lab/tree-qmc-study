...
df3 = df3.rename(columns={"STRHT": "HGHT",
                          "SRATE": "RATE",
                          "GTRE": "SUPP"})
ste_df = pandas.concat([df1, df2, df3])
...
for do in ["varyntax", "varyils"]: 
            ...
            for rate in rates:
                for ngen in ngens:
                    ...
                    for repl in repls:
                        ...
                        for mthd in mthds:
                            if mthd == "treeqmc_n2_v1.0.0":
                                supps = ["estimatedgenetre"]
                            elif mthd == "wtreeqmc_wn_n2":
                                supps = ["sh"]
                            else:
                                supps = ["sh", "abayes"]
                            ...
                            for supp in supps:
                                ...
                                row["RATE"] = rate
                                row["REPL"] = repl
                                if (mthd == "wtreeqmc_wn_n2") or (mthd == "treeqmc_n2_v1.0.0"):
                                    row["SUPP"] = "none"
                                else:
                                    row["SUPP"] = supp
                                row["NGEN"] = ngen
    ...
    df.to_csv("data-" + do + "-error-vs-original.csv",
