...
OUTF="$OUTDIR/true_species_tree_vs_true_gene_trees_${NBPS}.csv"
if [ ! -e $OUTF ]; then
...
OUTF="$OUTDIR/true_species_tree_vs_estimated_gene_trees_${NBPS}.csv"
if [ ! -e $OUTF ]; then
python3 $COMPARE_TREE_TO_LIST \
    -t $STRE_TRUE \
    -l $GTRE_ESTI \
...
OUTF="$OUTDIR/true_vs_estimated_gene_trees_${NBPS}.csv"
if [ ! -e $OUTF ]; then
