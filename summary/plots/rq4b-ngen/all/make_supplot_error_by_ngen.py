...
def make_figure(dfs, output):
    ...
    ax10 = plt.subplot(gs[1,0])  # avian esti
    ax20 = plt.subplot(gs[2,0])  # mammalian
    ...
    # Plot error for avian true gene trees
    ax = ax00
    sers = [None] * n
    nrps = [None] * n
    ...
    # Set labels
    #ax.set_title("Impact of Number of Gene Trees", 
    #             loc="center", y=1.175, fontsize=12)
    ax.set_title(upperletters[0] + " Avian 1X (48 taxa, true gene trees)",
                 loc="left", x=0.0, y=1.0, fontsize=11)
    ax.set_ylabel(r"Percent RF Error", fontsize=11)  

    ax.tick_params(axis='x', labelsize=10)
    ax.tick_params(axis='y', labelsize=9)
    #ax.set_xlabel("Number of Gene Trees", fontsize=11)
    ...
    # Plot error for avian estimated gene trees
    ax = ax10
    sers = [None] * n
    nrps = [None] * n
    for j, vary in enumerate(varys):
    ...
    # Set labels
    ax.set_title(upperletters[1] + " Avian 1X (48 taxa, estimated gene trees - 500 bp)",
                 loc="left", x=0.0, y=1.0, fontsize=11)
    ax.set_ylabel(r"Percent RF Error", fontsize=11)  
    ...
    # Plot error varying ngen for mammalian
    ax = ax20
    varys = ["25g", "50g", "100g", "200g", "400g", "800g"]
    vnams = ["25", "50", "100", "200", "400", "800"]
    n = len(varys)
    ...
    # Set labels
    ax.set_title(upperletters[2] + " Mammalian 1X (37 taxa, estimated gene trees - 500 bp)",
                 loc="left", x=0.0, y=1.0, fontsize=11)
    ax.set_ylabel(r"Percent RF Error", fontsize=11)  
    ...
    for j, vnam in enumerate(vnams):
        if nrps[j][0] != 20:
            sys.stdout.write("Mammalian %s - Found %d replicates!\n" % (vnam, nrps[j][0]))
        test.append(r"%s" % vnam)
    ax.set_xticklabels(test)
...
# Read and plot data
df = pandas.read_csv("../../../mahbub2021wqfm-avian/csvs/data-all-error-and-timings.csv")
df1 = df[(df["SCAL"] == "1X") &
         (df["NBPS"] == "true")]
df2 = df[(df["SCAL"] == "1X") &
         (df["NBPS"] == "500")]

df = pandas.read_csv("../../../mahbub2021wqfm-mammalian/csvs/data-all-error-and-timings.csv")
df3 = df[(df["SCAL"] == "noscale") &
         (df["NBPS"] == "500b")]

title = "supplot-rq4-ngen.pdf"
make_figure([df1, df2, df3], title)
